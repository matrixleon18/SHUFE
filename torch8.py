...
class Net(torch.nn.Module):
    ...
    def forward(self, x):
        x = torch.relu(self.hidden(x))                      # 这里作为函数调用，不会在类里显示这个方法名称
        x = self.predict(x)
        return x


net = Net(2, 10, 2)                                         # 输入两个值[横坐标,纵坐标]；输出onehot编码的两个值[0,1]或[1,0]表示两类
print(net)                                                  # 只会显示hidden和predict两个变量信息，也就是两个层的信息
...
net2 = torch.nn.Sequential(                             # 创建一个神经网络序列
    ...
    torch.nn.Linear(in_features=10, out_features=2),    # 再创建一个神经网络；输入１０个值，输出２个值。因为最后接受２个值
)
print(net2)